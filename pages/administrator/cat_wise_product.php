  ...
  <section class="content">
          <div class="row">
            <div class="col-md-12 text-center">
              <h3>Purchase Report Specific Category</h3>
            </div>
          </div>
          <hr>
          <div class="row">
            ...
            <div class="col-md-4">
              <div class="form-group">
                <label><b>Cat</b></label>
                <select  id="catId" onchange="ajaxCall()" class="form-control select2" required>
                  <?php
                  $run_items = mysqli_query($connection,"SELECT * FROM product_cat");
                  while ($row1=mysqli_fetch_array($run_items)) {

                  $product_id=$row1['id'];
                  $cat_name=$row1['cat_name'];
                  echo "<option value='$product_id'>$cat_name</option>";
                  } ?>
                </select>
              </div> 
            </div>
...
function export_all()
{
   ...
   $("#export_table").tableHTMLExport({
    type:'csv',
    filename:'Expenses_Report_'+Math.floor((Math.random() * 10000000) + 1)+'.csv',
  });
...
<script>
  ...
  function ajaxCall() 
  {
    ...
    $.ajax({
      ...
      data  : {
                purchase_prod_from1:from,
                purchase_prod_to1:to,
                catId:catId
                },
